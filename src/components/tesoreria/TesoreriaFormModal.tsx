...
export type TesoreriaFormModalProps = {
  open: boolean
  title: string
  subtitle?: string
  onClose: () => void
...
export function TesoreriaFormModal({
  open,
  title,
  subtitle,
  onClose,
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="tesoreria-form-modal-title"
        className={`flex max-h-[min(92vh,720px)] w-full ${maxWidthClass} flex-col overflow-hidden rounded-2xl border border-neutral-200/80 bg-white shadow-2xl`}
        onMouseDown={(e) => e.stopPropagation()}
      >
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-neutral-100 px-6 py-5">
          <div className="min-w-0">
            <h2
              id="tesoreria-form-modal-title"
